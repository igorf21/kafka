...
public class JoinStreamToTableWithFK {
    ...
    public Topology buildTopology(Properties envProps) {
        ...
        final KStream<Long, Album> albums = builder.stream(albumTopic, Consumed.with(longSerde, albumSerde));

        final KTable<Long, TrackPurchase> trackPurchases = builder.table(userTrackPurchaseTopic,
                Consumed.with(longSerde, trackPurchaseSerde));

        final MusicInterestJoiner trackJoiner = new MusicInterestJoiner();

        KStream<Long, MusicInterest> streamMusicInterest = albums.leftJoin(trackPurchases, TrackPurchase::getAlbumId, trackJoiner);
        streamMusicInterest.to("music-interest", Produced.with(longSerde,musicInterestSerde));
        streamMusicInterest.peek((key, value) -> System.out.println("Record: key=" + key + ", value=" + value));
        //streamMusicInterest.join(trackPurchases1,TrackPurchase::getAlbumId, trackJoiner);
    ...
    public static void main(String[] args) throws Exception {
        ...
        final JoinStreamToTableWithFK streamFkJoin = new JoinStreamToTableWithFK();
        final Properties envProps = streamFkJoin.loadEnvProperties(args[0]);
        final Properties streamProps = streamFkJoin.buildStreamsProperties(envProps);
        final Topology topology = streamFkJoin.buildTopology(envProps);
        System.out.println(topology.describe());

        streamFkJoin.createTopics(envProps);
