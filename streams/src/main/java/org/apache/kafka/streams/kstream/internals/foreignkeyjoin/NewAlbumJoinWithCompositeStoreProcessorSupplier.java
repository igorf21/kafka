...
public class NewAlbumJoinWithCompositeStoreProcessorSupplier<K, V, KO, VO, VR> implements
    ProcessorSupplier<KO, VO, K, SubscriptionResponseWrapper<VO>> {
    private static final Logger LOG = LoggerFactory.getLogger(NewAlbumJoinWithCompositeStoreProcessorSupplier.class);
    ...
    public NewAlbumJoinWithCompositeStoreProcessorSupplier(
    ...
    @Override
    public Processor<KO, VO, K, SubscriptionResponseWrapper<VO>> get() {
        return new KStreamKTableJoinProcessor();
    }


    private final class KStreamKTableJoinProcessor extends ContextualProcessor<KO, VO, K, SubscriptionResponseWrapper<VO>> {
        ...
        @Override
        public void process(final Record<KO, VO> record) {
            ...
            try (final KeyValueIterator<Bytes, ValueAndTimestamp<SubscriptionWrapper<K>>> prefixScanResults =
                ...
                while (prefixScanResults.hasNext()) {
                    ...
                    if (prefixEquals(next.key.get(), prefixBytes.get())) {
                        ...
                        SubscriptionResponseWrapper<VO> rw = new SubscriptionResponseWrapper<>
                                (next.value.value().getHash(),record.value(),next.value.value().getPrimaryPartition());

                        Record<K, SubscriptionResponseWrapper<VO>> newRecord =
                                record.withKey(combinedKey.getPrimaryKey()).withValue(rw);

                        final VR result = joiner.apply(null, record.value());
