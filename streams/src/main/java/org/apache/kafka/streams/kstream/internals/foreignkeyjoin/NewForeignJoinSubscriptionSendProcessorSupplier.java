...
import static org.apache.kafka.streams.kstream.internals.foreignkeyjoin.NewSubscriptionWrapper.Instruction.*;

public class NewForeignJoinSubscriptionSendProcessorSupplier<K, KO, V> implements ProcessorSupplier<K, Change<V>, KO, NewSubscriptionWrapper<K, V>> {
    private static final Logger LOG = LoggerFactory.getLogger(NewForeignJoinSubscriptionSendProcessorSupplier.class);
    ...
    public NewForeignJoinSubscriptionSendProcessorSupplier(final Function<V, KO> foreignKeyExtractor,
    ...
    @Override
    public Processor<K, Change<V>, KO, NewSubscriptionWrapper<K, V>> get() {
        return new UnbindChangeProcessor();
    }

    private class UnbindChangeProcessor extends ContextualProcessor<K, Change<V>, KO, NewSubscriptionWrapper<K, V>> {
        ...
        @Override
        public void init(final ProcessorContext<KO, NewSubscriptionWrapper<K, V>> context) {
            super.init(context);
        ...
        public void process(final Record<K, Change<V>> record) {
            ...
            if (record.value().oldValue != null) {
                    ...
                    if (!Arrays.equals(serialNewForeignKey, serialOldForeignKey)) {
                        ...
                        context().forward(
                            record.withKey(oldForeignKey)
                                .withValue(new NewSubscriptionWrapper<>(
                                    currentHash,
                ...
                final NewSubscriptionWrapper.Instruction instruction;
                if (leftJoin) {
                    ...
                    context().forward(
                        record.withKey(newForeignKey)
                            .withValue(new NewSubscriptionWrapper<>(
                                ...
                                instruction,
                                record.key(),
                                partition, record.value().newValue)));
                }
            }
        }
    }
