 ...
 */
public class NewCombinedKeySchema<K, KO> {
    ...
    private Serializer<KO> primaryKeySerializer;
    private Deserializer<KO> primaryKeyDeserializer;
    private Serializer<K> foreignKeySerializer;
    private Deserializer<K> foreignKeyDeserializer;

    public NewCombinedKeySchema(final Supplier<String> foreignKeySerdeTopicSupplier,
                                final Serde<K> foreignKeySerde,
                                final Supplier<String> primaryKeySerdeTopicSupplier,
                                final Serde<KO> primaryKeySerde) {
    ...
    public void init(final ProcessorContext<?, ?> context) {
        ...
        primaryKeySerializer = primaryKeySerializer == null ? (Serializer<KO>) context.keySerde().serializer() : primaryKeySerializer;
        primaryKeyDeserializer = primaryKeyDeserializer == null ? (Deserializer<KO>) context.keySerde().deserializer() : primaryKeyDeserializer;
        foreignKeySerializer = foreignKeySerializer == null ? (Serializer<K>) context.keySerde().serializer() : foreignKeySerializer;
        foreignKeyDeserializer = foreignKeyDeserializer == null ? (Deserializer<K>) context.keySerde().deserializer() : foreignKeyDeserializer;
    }

    Bytes toBytes(final K foreignKey, final KO primaryKey) {
    ...
    public CombinedKey<KO, K> fromBytes(final Bytes data) {
        ...
        final K foreignKey = foreignKeyDeserializer.deserialize(foreignKeySerdeTopic, foreignKeyRaw);
        ...
        final KO primaryKey = primaryKeyDeserializer.deserialize(primaryKeySerdeTopic, primaryKeyRaw);
        return new CombinedKey<>(primaryKey,foreignKey);
    }

    Bytes prefixBytes(final K key) {
